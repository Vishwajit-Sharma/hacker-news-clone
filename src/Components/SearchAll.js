...
export default function SearchDefault(props) {
  ...
  useEffect(()=>{
    const fetchNews = async () =>{
      const url = props.secondFilter === "popularity" ? `https://hn.algolia.com/api/v1/search?query=${props.search}&page=${page}&hitsPerPage=50&numericFilters=${timeFilter}` : `https://hn.algolia.com/api/v1/search_by_date?query=${props.search}&page=${page}&hitsPerPage=50&numericFilters=${timeFilter}`
    ...
    fetchNews()
  },[page,props.search, props.secondFilter, props.thirdFilter])
